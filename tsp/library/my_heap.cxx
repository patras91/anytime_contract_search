...
class Heap {
 ...
 private:  
  ...
  void heapify_down(int index) {     
	...
	if (l) {
      TspState* tmp = heap[index];
      TspState* s = heap[lchild];
      heap[index] = s;
      heap[lchild] = tmp;
      si[s] = index;
      si[tmp] = lchild;
  ...
  vector<TspState*> heap;
  short _comparison_flag;
  map <TspState*, int> si;  // state vs heap index
};
