...
PuzState * APGenerateChildrenGraphHelper(int i, PuzState * state, myPriorityQueue * CP, HHashtable * cl, HHeap * sus, myPriorityQueue * OP, int res, HHashtable *qh, HHashtable *ch, HHashtable *sush) {
...
void APGenerateChildrenGraphK(PuzState * state, myPriorityQueue * CP, HHashtable * cl, HHeap * sus, myPriorityQueue * OP, int res, int K, HHashtable *qh, HHashtable *ch, HHashtable *sush) {
	NUM_NODES_EXPANDED++;
  myPriorityQueue* q = CP;
  for (int i = 0; i < 4; i++) {
	PuzState *childstate = APGenerateChildrenGraphHelper(i, state, CP, cl, sus, OP, res, qh, ch, sush);
    if (childstate) {
...
/*
void APGenerateChildrenGraph(PuzState * state, myPriorityQueue * CP, HHeap * clo, HHeap * sus, myPriorityQueue * OP, int res) {
  for (int i = 0; i < 4; i++) {
	PuzState *childstate = APGenerateChildrenGraphHelper(i, state, CP, clo, sus, OP, res);
    if (childstate) {
      CP->push (childstate);
    }
  }
